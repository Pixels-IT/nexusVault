...
export default function Navbar() {
  ...
  const isActive = (path, end = false) => end ? location.pathname === path : location.pathname.startsWith(path);
    ...
    <nav className="navbar">
      <div className="navbar-inner">
        ...
        <div className="nav-items">
          ...
          <button
            className={`nav-item ${isActive('/backups') ? 'active' : ''}` }
          ...
          <button
            className={`nav-item ${isActive('/automatisation') ? 'active' : ''}` }
            onClick={() => navigate('/automatisation')}
            style={{ background: 'none', border: 'none', cursor: 'pointer', font: 'inherit', display: 'flex', alignItems: 'center', gap: 6 }}
          >
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
